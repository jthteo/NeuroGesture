...
console.log('Running postural recording for palm (position, velocity & vector) and finger (position, velocity & vector)');

var now = new Date();
var dhour = now.getHours();
var dminute = now.getMinutes();

var palm_pos_stream=fs.createWriteStream('Postural_palm_position_'+dhour+dminute+'hrs.csv',{flags:'w'});
var palm_velocity_stream=fs.createWriteStream('Postural_palm_velocity_'+dhour+dminute+'hrs.csv',{flags:'w'});
var finger_pos_stream = fs.createWriteStream('Postural_finger_position_'+dhour+dminute+'hrs.csv', {flags : 'w'});
var finger_velocity_stream = fs.createWriteStream('Postural_finger_velocity_'+dhour+dminute+'hrs.csv', {flags : 'w'});
var palm_direction_stream=fs.createWriteStream('Postural_palm_direction_'+dhour+dminute+'hrs.csv',{flags:'w'}); //// new line
var finger_vector_stream = fs.createWriteStream('Postural_finger_vector_'+dhour+dminute+'hrs.csv', {flags : 'w'}); //// new line
// var palm_rot_stream=fs.createWriteStream('postural_palm_rotation.csv',{flags:'w'});
